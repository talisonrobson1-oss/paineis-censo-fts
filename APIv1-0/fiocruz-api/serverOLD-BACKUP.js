...
const pool = new Pool({
  ...
  max: 40, // Aumentado para 40 conexões
...
function buildResolucaoWhere(filters) {
  ...
  if (filters.operacao) {
    ...
    paramIndex++;
    console.log(`  ✓ Filtro Operação aplicado: ${filters.operacao}`);
  }
...
app.get('/api/resolucao/stats', async (req, res) => {
  try {
    ...
    // CHAVE ÚNICA DE VÍNCULO: 7 campos
    ...
    const CHAVE_VINCULO = `nu_cpf || '|' || co_cnes || '|' || co_cbo_ocupacao || '|' || nu_vinculacao || '|' || COALESCE(qt_carga_horaria_ambulatorial, '') || '|' || COALESCE(qt_carga_horaria_hospitalar, '') || '|' || COALESCE(qt_carga_horaria_outros, '')
      COALESCE(qt_carga_horaria_ambulatorial, '')
      COALESCE(qt_carga_horaria_hospitalar, '')
      COALESCE(qt_carga_horaria_outros, '')`;
    ...
    const CHAVE_V = `v.nu_cpf || '|' || v.co_cnes || '|' || v.co_cbo_ocupacao || '|' || v.nu_vinculacao || '|' || COALESCE(v.qt_carga_horaria_ambulatorial, '') || '|' || COALESCE(v.qt_carga_horaria_hospitalar, '') || '|' || COALESCE(v.qt_carga_horaria_outros, '')
      COALESCE(v.qt_carga_horaria_ambulatorial, '')
      COALESCE(v.qt_carga_horaria_hospitalar, '')
      COALESCE(v.qt_carga_horaria_outros, '')`;

    // Total de divergências (TODOS os vínculos únicos - chave de 7 campos)
    const totalDivQuery = `
      SELECT COUNT(DISTINCT ${CHAVE_VINCULO}) as total
      FROM censo.vinculos
      WHERE no_tipo_operacao_censo IS NOT NULL
    `;

    // Divergências resolvidas (chave de 7 campos)
    const resolvidasQuery = `
      ...
      INNER JOIN censo.espelho_cnes e 
        ...
        ${compFilter}
      WHERE v.no_tipo_operacao_censo IS NOT NULL
    `;
    ...
    const [totalDiv, resolvidas, competencias] = await Promise.all([
      params.length > 0 ? pool.query(totalDivQuery, params) : pool.query(totalDivQuery),
      params.length > 0 ? pool.query(resolvidasQuery, params) : pool.query(resolvidasQuery),
      pool.query(competenciasQuery)
    ]);

    const total = parseInt(totalDiv.rows[0].total);
    const totalResolvidas = parseInt(resolvidas.rows[0].resolvidas);
  ...
  try {
    ...
    const evolucaoQuery = `
      WITH total_vinculos AS (
        SELECT COUNT(DISTINCT ${CHAVE_VINCULO.replace(/\n/g, ' ')}) as total
        FROM censo.vinculos
        WHERE no_tipo_operacao_censo IS NOT NULL
      )
      SELECT 
    ...
    const evolucaoAcumuladaQuery = `
      WITH total_vinculos AS (
        SELECT COUNT(DISTINCT nu_cpf || '|' || co_cnes || '|' || co_cbo_ocupacao || '|' || nu_vinculacao || '|' || COALESCE(qt_carga_horaria_ambulatorial, '') || '|' || COALESCE(qt_carga_horaria_hospitalar, '') || '|' || COALESCE(qt_carga_horaria_outros, '')) as total
        FROM censo.vinculos
        WHERE no_tipo_operacao_censo IS NOT NULL
      ),
      ...
      competencias AS (
        SELECT DISTINCT nu_comp FROM censo.espelho_cnes ORDER BY nu_comp
      )
      SELECT 
        c.nu_comp as competencia,
        tv.total,
        -- Conta vínculos resolvidos até esta competência (acumulado)
        COUNT(DISTINCT CASE WHEN pr.comp_resolucao <= c.nu_comp THEN pr.id END) as resolvidas_acumuladas
      FROM competencias c
      CROSS JOIN total_vinculos tv
      LEFT JOIN primeira_resolucao pr ON 1=1
    ...
    const cboPendentesQuery = `
      ...
      WHERE v.no_tipo_operacao_censo IS NOT NULL
        ...
        AND v.ds_cbo_ocupacao != ''
      GROUP BY v.ds_cbo_ocupacao
      HAVING COUNT(DISTINCT v.nu_cpf || '|' || v.co_cnes || '|' || v.co_cbo_ocupacao || '|' || v.nu_vinculacao || '|' || COALESCE(v.qt_carga_horaria_ambulatorial, '') || '|' || COALESCE(v.qt_carga_horaria_hospitalar, '') || '|' || COALESCE(v.qt_carga_horaria_outros, '')) >= 10
    ...
    const [evolucao, tipo, evolucaoAcumulada, cbo, cnes, cboPendentes, cnesPendentes] = await Promise.all([
      params.length > 0 ? pool.query(evolucaoQuery, params) : pool.query(evolucaoQuery),
      params.length > 0 ? pool.query(tipoQuery, params) : pool.query(tipoQuery),
      pool.query(evolucaoAcumuladaQuery), // Sem params - sempre total
      params.length > 0 ? pool.query(cboQuery, params) : pool.query(cboQuery),
      params.length > 0 ? pool.query(cnesQuery, params) : pool.query(cnesQuery),
      params.length > 0 ? pool.query(cboPendentesQuery, params) : pool.query(cboPendentesQuery),
      params.length > 0 ? pool.query(cnesPendentesQuery, params) : pool.query(cnesPendentesQuery)
    ]);
